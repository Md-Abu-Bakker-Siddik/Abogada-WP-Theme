...
class AbogadaOneClickImport {
	...
	function __construct() {
		...
		//FTP
		$this->revSliderFolder_importURI = 'https://kodesolution.live/one-click-demo-import/abogada/import-rev-sliders/';
		$this->base_url = 'https://kodesolution.live/one-click-demo-import/abogada/v1.0/';

		//HTTP
		$this->preview_base_url = 'https://dev243.kodesolution.com/abogada/';
			...
			$final_output[] = array(
					...
					'import_redux'               => array(
							array(
									...
									'option_name' => 'abogada_redux_theme_opt',
							),
					),
					'import_preview_image_url'   => $current_url . 'screenshot.jpg',
					'import_notice'              => esc_html__( 'After you import this demo, you will have to import the slider separately. You will find slider in "import-sliders" folder in your downloaded package.', 'abogada' ),
	...
	public function after_import_setup( $selected_import ) {
		...
		abogada_generate_dynamic_css();
		abogada_generate_css_for_custom_theme_color_from_scss();
	...
	public function plugin_page_setup( $default_settings ) {
		...
		$default_settings['page_title']  = esc_html__( 'One Click Demo Import - ThemeMascot' , 'abogada' );
		$default_settings['menu_title']  = esc_html__( 'Import Demo Data' , 'abogada' );
					...
					foreach ( $demo_rev_sliders as $demo_rev_slider ) {
						...
						if ( ! in_array( str_replace( '.zip', '', $demo_rev_slider ), $rev_sliders, true ) ) {
							...
							if ( ! $rev_slider_result['success'] ) {
								esc_html_e( 'Error while importing rev sliders', 'abogada' );
								exit;
							}
						}
					}

					esc_html_e( 'Rev sliders imported successfully', 'abogada' );
				} else {
					esc_html_e( 'RevSliderSliderImport instance is empty', 'abogada' );
				}

			} else {
				esc_html_e( 'RevSliderSliderImport class doesn\'t exist', 'abogada' );
			}

		} else {
			esc_html_e( 'Files don\'t exist', 'abogada' );
		}
	}

}

AbogadaOneClickImport::get_instance();